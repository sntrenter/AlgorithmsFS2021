...
//
//There is a version with classes defined inline that works with the makefile
//  this however just uses the command below
//
//example of how to run:
// g++ gifting.cpp Child.cpp Gift.cpp Node.cpp -std=c++11 -o main ; ./main.exe "examples/ex1_5child_14gifts.txt" "writefile" 5 14
//Clark has c++11 so it should have no issues running
...
void distGifts(string writeFile)
{
    ...
    */
    ...
    cout << "Size of the Nodes list:" << endl;
        ...
        for (auto n : nodes)
        {
            for (auto c : children)
            {
                if (canRecieveGift(g, c)) // || (i>8) )
                {
                    ...
                    //if (inGiftRange(n, maxGifts)) //Due to implementation
                    //{                             //inGiftRange() is to slow to help.
                    newNodes.insert(nNit, n);       //Could be enabled for very
                    //}                             //large sets though.
                }
            }
        }
